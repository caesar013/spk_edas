...
<script>
    ...
    $('.table-title').html('<b>Average for ' + "{{ $edas->name }}<b>");
    ...
    $('#href_apraisalscore').attr('href' , "/dashboard/apraisalscore/"+id_edas+ "");
        ...
        if (averages.length==0) {
                    $('tbody').append(`
                    <tr>
                        <td colspan="2" class="text-center"> No Averages tracked. Complete Decision Matrix Table first.</td>
                    </tr>`);
        } else {
            $.each(averages, function(foo, bar) {
                $('tbody').append(`
                    <tr>
                        <td><b> AV` + (foo+1) + `</b></td>
                        <td>` + bar.value + `</td>
