...
<script>
    ...
    $('.table-title').html('<b>PDA for ' + "{{ $edas->name }}<b>");
    ...
    $('#href_apraisalscore').attr('href' , "/dashboard/apraisalscore/"+id_edas+ "");
        ...
        if (pdas.length==0) {
                    $('tbody').append(`
                    <tr>
                        <td colspan="2" class="text-center"> No PDA tracked. Complete Decision Matrix Table first.</td>
                    </tr>`);
        } else {
            let head = `<tr><th></th>`;
            ...
            alternatives.forEach(alternative => {
                ...
                criterias.forEach(criteria => {
                    let pda_value = pdas.find(pda => pda.id_criteria == criteria.id && pda.id_alternative == alternative.id).value || null;
                    row += `<td>` + pda_value + `</td>`;
                });
                row += `</tr>`;
            });
